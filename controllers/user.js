...
exports.LoginUser = async (req, res) => {
    ...
    try {
        ...
        const verifyPassword = await VerifyHashPassword(password, findUser.password);
        if (!verifyPassword) {
...
exports.UpdateUserData = async (req, res) => {
    ...
    try {
        const updateUser = await userModel.findByIdAndUpdate(tokenData.id, { ...data });
...
exports.GetUserProfile = async (req, res) => {
    ...
    try {
        const findUser = await userModel.findById(tokenData.id, { password: 0, verified: 0, token: 0 });
        if (!findUser) {
...
exports.GetAnotherUserProfile = async (req, res) => {
    ...
    try {
        const searchUser = await userModel.findById(userId, { password: 0, verified: 0, token: 0, saved: 0, liked: 0 });
        if (!searchUser) {
...
exports.UpdateSaved = async (req, res) => {
    ...
    try {
        ...
        if (findUser.saved.includes(blogId)) {
            ...
            const updateSaved = await userModel.findByIdAndUpdate(findUser._id, { saved: removeSavedBlog });
        ...
        const saveBlog = await userModel.findByIdAndUpdate(findUser._id, { saved: [...findUser.saved, findBlog._id] });
...
exports.UpdateFollowers = async (req, res) => {
    ...
    try {
        ...
        if (Request.followers.includes(Sender._id)) {
            console.log('done')
            const indexOfSenderInRequest = Request.followers.indexOf(Sender._id)
            const removeSenderFromRequest = Request.followers.filter((items, index) => {
                return items !== Sender._id && index !== indexOfSenderInRequest;
            })
            const updateRequestFollowers = await userModel.findByIdAndUpdate(Request._id, { followers: removeSenderFromRequest });

            const indexOfRequestInSender = Sender.following.indexOf(Request._id);
            const removeRequestFromSender = Sender.following.filter((items, index) => {
                return items !== Request._id && index !== indexOfRequestInSender;
            })
            const updateSenderFollowing = await userModel.findByIdAndUpdate(Sender._id, { following: removeRequestFromSender });
        ...
        const addFollowingInSender = await userModel.findByIdAndUpdate(Sender._id, { following: [...Sender.following, Request._id] });
        const addFollowersInRequest = await userModel.findByIdAndUpdate(Request._id, { followers: [...Request.followers, Sender._id] });
